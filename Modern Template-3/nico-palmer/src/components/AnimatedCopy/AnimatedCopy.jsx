...
const AnimatedCopy = ({
  ...
  useGSAP(
    () => {
      ...
      // Safety: if the timeline never plays (throttled tab / ScrollTrigger miss),
      // snap lines into view so type is never left off-screen.
      const safetyMs = Math.ceil((delay + duration + 0.5) * 1000) + 800;
      const safetyId = window.setTimeout(() => {
        gsap.set(`.line-inner-${copyId}`, { y: "0%" });
      }, safetyMs);
